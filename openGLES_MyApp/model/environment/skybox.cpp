...
void SkyBox::InitFront(const char *imageDir) {
	mShader[0].Init("skybox.vs", "skybox.fs");
	char temp[256];
	memset(temp, 0, 256);
...
void SkyBox::InitBack(const char *imageDir) {
	mShader[1].Init("skybox.vs", "skybox.fs");
	char temp[256];
	memset(temp, 0, 256);
//    strcpy(temp, imageDir);
...
void SkyBox::InitLeft(const char *imageDir) {
	mShader[2].Init("skybox.vs", "skybox.fs");
	char temp[256];
	memset(temp, 0, 256);
//    strcpy(temp, imageDir);
...
void SkyBox::InitRight(const char *imageDir) {
	mShader[3].Init("skybox.vs", "skybox.fs");
	char temp[256];
	memset(temp, 0, 256);
//    strcpy(temp, imageDir);
...
void SkyBox::InitTop(const char *imageDir) {
	mShader[4].Init("skybox.vs", "skybox.fs");
	char temp[256];
	memset(temp, 0, 256);
//    strcpy(temp, imageDir);
	strcat(temp, "top.bmp");
...
void SkyBox::InitBottom(const char *imageDir) {
	mShader[5].Init("skybox.vs", "skybox.fs");
	char temp[256];
	memset(temp, 0, 256);
//    strcpy(temp, imageDir);
...
void SkyBox::Draw(float x, float y, float z, glm::mat4 &V, glm::mat4&P) {
	glDisable(GL_DEPTH_TEST);
    glm::mat4 modelMatrix = glm::translate(x, y, z);
	for (int i=0;i<6;++i){
		mVertexBuffer[i].Bind();
		mShader[i].Bind(glm::value_ptr(modelMatrix), glm::value_ptr(V), glm::value_ptr(P));
